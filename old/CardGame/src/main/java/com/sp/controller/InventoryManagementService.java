...
public class InventoryManagementService {
    ...
    // public Boolean addToUserInventoryDTO(String user_id, Integer card_id){
    // !!!! TO DELETE TEMPORARY SOLUTION !!!!
    public Boolean addToUserInventoryDTO(User user , Integer card_id) {
        // Get user's inventory
        Inventory inventory =  iRepository.findByUser(user);
        Card card = cardManagementService.getCardById(card_id);

        // Check if he hasn't this card yet
        if (inventory.hasCard(card_id)) {
            System.err.println("AlreadyHasCard");
            return false;
        }
        inventory.addCard(card);
    ...
    public Boolean removeFromUserInventory(User user, Integer card_id) {
        // Get user's inventory       
        Inventory inventory =  iRepository.findByUser(user);
        Card card = cardManagementService.getCardById(card_id);
