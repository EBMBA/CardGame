...
public class CardManagementRestCrt {
   ...
   public List<CardDTO> getCards() {
    ...
    public ResponseEntity<Object> getRandomCard() {
        Set<Integer> card = cardDisplayService.getRandomCardID(1);
            
        if (card != null) {
            return ResponseEntity.ok(card.toArray()[0]);
        }
        
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Error in the process");
    }
